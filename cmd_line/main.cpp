...
static void fit(struct data_set& X,
         ...
         double lambda, double psi, double phi,
         double threshold,
         uint32_t maxit,
         uint32_t nfolds);
...
int main(int argc, char *argv[])
{
    ...
    boost::program_options::options_description mods(
        "Model selection");
    mods.add_options()
            ("modelselection",
            ...
            ("epsilon,e",
             boost::program_options::value<decltype(epsilon)>(
    ...
    {
        if (X.nrow > 1000 || X.ncol > 500) nfolds = 5;
...
void fit(struct data_set& X,
         ...
         uint32_t nfolds)
{
    netreg::graph_penalized_linear_model_data dat = get_fit_data(
    ...
    arma::Col<double> intr = netreg::intercept(dat.design(),
                                               dat.response(),
                                               coef);
...
std::map<std::string, double> modelselection(
    struct data_set& X,
    struct data_set& Y,
    const std::string& gx_filename,
    const std::string& gy_filename,
...
{
    netreg::graph_penalized_linear_model_cv_data dat = get_ms_data(
        X, Y, gx_filename, gy_filename,
        lambda, psi, phi, threshold, maxit, nfolds
    );
    ...
    uint32_t nfolds)
{
    if (gx_filename != "" && gy_filename != "")
    {
        ...
        return netreg::graph_penalized_linear_model_data(
            X.data.data(),
            Y.data.data(),
            GX.data.data(), GY.data.data(), X.nrow, X.ncol, Y.ncol,
            lambda, 1.0, psi, phi, maxit, threshold,
    ...
    else if (gx_filename != "")
    {
        ...
        return netreg::graph_penalized_linear_model_data(
            ...
            lambda, 1.0, psi, 0, maxit, threshold,
            netreg::family::GAUSSIAN);
    }
    else if (gy_filename != "")
    {
        ...
        return netreg::graph_penalized_linear_model_data(
            ...
            X.nrow, X.ncol, Y.ncol,
            lambda, 1.0, 0, phi, maxit, threshold,
            netreg::family::GAUSSIAN);
    }
    else
    {
        return netreg::graph_penalized_linear_model_data(
            ...
            X.nrow, X.ncol, Y.ncol,
            lambda, 1.0, 0, 0, maxit, threshold,
            netreg::family::GAUSSIAN);
    }
}
    ...
    uint32_t nfolds)
{
    ...
    {
        return netreg::graph_penalized_linear_model_cv_data(X.data.data(),
                                                            Y.data.data(),
                                                            std::vector<double>(
                                                                1).data(),
                                                            std::vector<double>(
