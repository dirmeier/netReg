...
namespace netreg
{
     ...
     */
    class graph_penalized_linear_model_data : public penalized_linear_model_data
    {
       public:
         ...
         */
        graph_penalized_linear_model_data(double* const x,
                                          double* const y,
                                          double* const gx,
                                          double* const gy,
                                          const int n,
            ...
            : penalized_linear_model_data(x,
                                          ...
                                          thresh,
                                          fam),
              psi_gx(psi_gx), psi_gy(psi_gy), GX(gx, p, p, false, true),
              GY(gy, q, q, false, true), LX(laplacian(gx, p, p, psi_gx)),
              LY(laplacian(gy, q, q, psi_gy)), lx_rows_(LX.n_rows)
        {
            // copies LX rows as single vectors so that access can be faster
            #pragma omp parallel for
            for (std::vector<arma::Row<double>>::size_type i = 0; i < LX.n_rows;
                 ++i)
            {
                lx_rows_[i] = LX.row(i);
            }
