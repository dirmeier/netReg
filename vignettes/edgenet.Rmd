...
This vignette covers penalization using `edgenet` as introduced in @dirmeier2018netreg. `edgenet` uses networks to model pair-wise interactions of covariables and responses in generalized linear regression models. More specifically, `edgenet`
...
So far `edgenet` supports the following families:
...
Since $\mathbf{Y}$ is multivariate, we factorize the likelihood over every component $Y_j$ as:

\begin{equation}
\small
\ell(\mathbf{B}) = \sum_{j}^q \sum_i^n  \mathbb{P}\left({y}_{i, j} \mid h\left(\mathbf{x}_{i,*} \cdot \beta_{*,j}\right)\right),
\end{equation}

where $h$ is the inverse of a link function, such as the logarithm.
