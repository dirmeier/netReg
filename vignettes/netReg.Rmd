...
to model interactions of $q$ response variables in $\mathbf{Y} \in \mathbb{R}^{n \times q}$. The affinity matrices are used for regularization of regression coefficients like this:
...
\hat{\mathbf{B}} = \arg \min_{\mathbf{B}} \,
 & -\ell(\mathbf{B}) + \lambda ||\mathbf{B} ||_1  \\  & +
      		  \frac{\psi_1}{2}  \sum_{i < j}^p
      		   || \boldsymbol \beta_{i,*} - \boldsymbol \beta_{j,*} ||_2^2 (\mathbf{G}_X)_{i,j}    \\  & +
 \frac{\psi_2}{2}  \sum_{i < j}^q || \boldsymbol \beta_{*,i} - \boldsymbol \beta_{*,j} ||_2^2 (\mathbf{G}_Y)_{i,j} \\
 = \arg \min_{\mathbf{B}} \,
 &  -\ell(\mathbf{B})  + \lambda ||\mathbf{B} ||_1  \\  & +
...
Matrix $\mathbf{B} \in \mathbb{R}^{p \times q}$ is the matrix of regression coefficients to be estimated.
...
Using `group.lasso` estimates the coefficients as:

\begin{equation}
\small
\begin{split}
\hat{\mathbf{B}} = \arg \min_{\mathbf{B}}-\ell(\mathbf{B})  + \lambda q \sum_{g=1}^G \sqrt{p_{\mathcal{I}_g} } ||\mathbf{B}_{\mathcal{I}_g} ||_2
\end{split}
\end{equation}

where $\mathcal{I}_g$ is a set of indexes belonging to the $g$th group of variables and  $p_{\mathcal{I}_g}$ is the number of indexes in $I_g$.

For details see @yuan2006model and @meier2008group.
