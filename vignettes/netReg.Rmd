...
<<<<<<< HEAD
`netReg` provides an efficient implementation of these graph-penalized 
=======
`netReg` provides a highly-efficient implementation of these graph-penalized 
>>>>>>> upstream/master
regression model. The models introduce a priori generated biological graph 
...
<<<<<<< HEAD
[@friedman2010regularization]. The package is an `R`-wrapper to an external `C++` 
library that uses `RcppArmadillo` [@eddelbuettel2014rcpparmadillo] for 
fast matrix calculations and `Dlib` [@king2009dlib] for gradient-free 
=======
[@friedman2010regularization]. The package is an R-wrapper to an external C++ 
library that uses \texttt{RcppArmadillo} [@eddelbuettel2014rcpparmadillo] for 
fast matrix calculations and \texttt{dlib} [@king2009dlib] for gradient-free 
>>>>>>> upstream/master
