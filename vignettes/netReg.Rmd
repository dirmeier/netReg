...
`netReg` computes coefficients using \textit{cyclic coordinate descent} as previously introduced [@fu1998penalized, @friedman2007pathwise, @friedman2010regularization]. The package is an R-wrapper to an external C++ library that uses \texttt{RcppArmadillo} [@eddelbuettel2014rcpparmadillo] for fast matrix calculations and \texttt{dlib} [@king2009dlib] for gradient-free convex optimization for model selection.

## Edgenet tutorial

This section explains how to fit a linear model and do parameter estimation using `edgenet`-regularization. The model is a truncated version from [@cheng2014graph] that is able to introduce prior graphs for the design and response matrices for penalization.
...
We use Powell's BOBYQA algorithm ([@powell2009bobyqa]) for gradient-free optimization that is included in the `C++` library `Dlib`. Doing the model selection only requires calling `cv.edgnet`:
